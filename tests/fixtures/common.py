...
def server():
    ...
    app.store.vk_api.send_event = AsyncMock()
    app.store.rabbit = AsyncMock()
    # app.store.rabbit.connect = AsyncMock()
    # app.store.rabbit.disconnect = AsyncMock()
    # app.store.rabbit.on_message = AsyncMock()
    # app.store.rabbit.rabbit_produce = AsyncMock()
...
async def admin(cli, db_session, config: Config) -> Admin:
    new_admin = AdminModel(
        ...
        vk_id=config.admin.vk_id,
    )
    async with db_session.begin() as session:
        session.add(new_admin)

    return Admin(id=new_admin.id, email=new_admin.email, vk_id=new_admin.vk_id)
